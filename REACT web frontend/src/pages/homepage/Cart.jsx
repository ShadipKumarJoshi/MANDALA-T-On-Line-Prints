...
const Cart = () => {
    ...
    const handleQuantityChange = (value, cart) => {
        ...
        const updatedCartItems = cartItems.map((item) =>
            item._id === cart._id
                ? {
                    ...item,
                    quantity: value,
                    total: item.customizePrice * value,
                }
                : item
        );
        ...
        const data = {
            quantity: value,
            total: cart.customizePrice * value,
        };
                        ...
                        <AnimatePresence>
                            {cartItems.map((cart, index) => (
                                ...
                                >
                                    <Image
                                        width={150}
                                        src={`http://localhost:8000/designs/${cart.designId.designImage}`}
                                        alt={cart.designName}
                                        preview={{
                                            src: `http://localhost:8000/designs/${cart.designId.designImage}`,
                                        }}
                                    />
                                    <ItemDetails>
                                        <ItemName>Category: {cart.productId.productCategory}</ItemName>
                                        <ItemName>Size: {cart.productSize}</ItemName>
                                        <ItemName>Color: {cart.productColor}</ItemName>
                                        <ItemName>Design: {cart.designId.designName}</ItemName>
                                        <ItemName>Design ID: {cart.designId._id}</ItemName>
                                        <ItemPrice>Rs. {cart.customizePrice}</ItemPrice>
